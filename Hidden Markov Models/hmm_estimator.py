...
if __name__ == '__main__':
   ...
   #'Two states, three possible observations in a state'
    ...
    # Here we define the HMM, with known parameters. The purpose would be clear later.
    hmm = HMM()
    ...
    # Randomly initiate an HMM, weights to be trained later by FB & Viterbi
    hmmguess = HMM()
    hmmguess.pi = np.array([0.4, 0.6])
    hmmguess.A = np.random.random([2,2])
    hmmguess.B = np.random.random([2,3])
    # #three states
    ...
    o,s = hmm.simulate(1000) # Generate samples from the first HMM, to train the second :p
    # Just for validating
